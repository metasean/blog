...
You really don't need a lot of prerequisite knowledge to follow along with this post.  

Web Console::
You do need to be able to use your browser's web console and scratchpad.  If you aren't familiar with your browser's console and scratchpad, wait for the video version of this post.

JavaScript::
For the version I'm going to show you, you need to be minimally familiar with the following concepts.  
...
As we proceed through this post, we'll actually touch on a lot of other aspects of JS, including if statements, equality, and objects.  But, you do _not_ need to know anything about these to follow along. I'll either explicitly explain what I'm doing or I'll assume that the context makes it clear what's going on (where I've done that, I've also provided links to relevant documentation, so that if you want to dive into it more you'll already be standing on a good diving board).
...
Imagine you're teaching a class of kids how to do subtraction for the very first time.  After giving them the introduction, you write "3 - 1 =" up on the board, then have each kid write their answer really big on a sheet of paper and hold it up.  Most kids hold up a sheet with the number "2" in it, but a few hold up a sheet with the number "4" on it.  You now have a snapshot of the whole class and know which students don't quite have the concept.  TDD is kind of like this, but for you and your code.



### Test Pseudo-code and Example

Our primary goal is to be able to write a basic.  A basic test looks something like like:
...
Imagine we have to write an `add` function, that takes two parameters and returns their sum.  Using the example above, what might such a test look like?
...
If you notice, I've made a call to a `test` function, but that function doesn't exist - _yet!_  We'll come back to this in just a moment.  First, we're going to take a really quick detour into some terminology and styles.

### TDD Frameworks and Testing Types

First, lets get a high lvel overviwe of sme key concepts and terms.

### Basic Concepts

Different groups may have important nuances among their use of these terms. 
...
Assertion Styles::
Assertion styles specify the http://chaijs.com/guide/styles/[lingo] that will be used to construct tests.  There are three primary styles: assert, expect, and should.
+
  * assert style test: `assert.equal( add(1,1), 2, '1+1 = 2' );`
  * expect style test: `expect.( add(1,1), '1+1 = 2' ).to.equal( 2 );`
  * should style test: `add(1,1).should.equal( 2 );`
...
Once you have the test and code working as stated, then you can edit your code to be more clear, more concise, more precise, etc.  This is known as "refactoring" the code.  One of the awesome things about TDD is that you can refactor your code with the confidence that _if you break functionality that has a test - your tests will fail, thereby alerting you that there is a problem_. 

## Implementing a Learners' Level Test Framework

Okay, now for the fun part!

We're going to iterate through writing our own test framework.  It will of course be very simple, but by writing it yourself, you can (a) easily test your own functions without being dependent on a specific test framework, (b) customize it to your heart's content, and (c) expand it as your needs grow.

//// 
TODO: add git stuff here

If you're familiar with git, you should go ahead and start a repo, then commit after each version below.
...
We're going to start with the test style from our pseudo-code above.  
...
First, let's review our error message.
...
If you run that, you should get your message logged to the console twice.  In other words, `test` is doing exactly what we tell it to at this point. 
...
If you run your code now, you should get one 'true' and one 'false' - that's _almost_ what we want.  In other words, we're on the right track!  _Importantly, it's close enough that you could actually use it just like this if you wanted or needed to do so!_

But it's much more clear to have an actual "pass" or "fail".  We can accomplish this several different ways.  For reasons that may become clear, I'm going to steer us toward an `if` approach.  So, let's implement a basic `if` check and then log either "pass" or "fail".
...
Let me show you a https://developer.mozilla.org/en-US/docs/Web/API/Console#Styling_console_output[cool thing] about most modern browsers – something that's particularly relevant for our test's output:
...
If you guessed that it colors passing log messages green and failing log messages red - great guess!
...
Unfortunately, the answer is kind of a rabbit hole –a great rabbit hole– but a rabbit hole nonetheless.  Since I don't want to go down that rabbit hole in this post, I will instead highly recommend that you https://metasean.github.io/blog/2016/04/02/J-S-Learners-Fieldguide.html#_dive_deep[Dive Deep] into JavaScript sometime soon.
====

When you re-run your scratchpad, you should experience more than a little bit of déjà vu as you read the "`Test.assertEquals is not defined`" error.  But before you despair too much, note that error message actually includes the `assertEquals` part, which means it's really a different error. ;-)

Before we address that, let's play around for just a bit.  Specifically, add the following code after your empty `Test` function and then run your code.
...
Before I go any further, try to work through what you think the code is doing, and then try to modify your `test` function so that your tests work.
...
Test.assertEquals( 3.14, 6.28, 'my failing condition');
----
And that's mostly it.  At this point, you can now write lots of strict equality tests and get useful information back.  Even better, you should be able to:

* Write a super quick, super simple test function in JavaScript!
* Modify your version of `Test.assertEquals`!
* Add new tests to your `Test` object to your heart's content!

*Consider this as my gift to you.  Please enjoy doing awesome magic with your very first code wizard's wand!*
...
https://www.browserstack.com/[BrowserStack], https://saucelabs.com/[Sauce Labs], and https://testingbot.com/[Testingbot]::
